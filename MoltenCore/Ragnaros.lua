...
function mod:OnEngage()
	...
	self:Message("submerge", "Attention", nil, CL.custom_min:format(L.submerge, 3), "misc_arrowdown")
	self:DelayedMessage("submerge", 60, "Attention", CL.custom_min:format(L.submerge, 2))
	self:DelayedMessage("submerge", 120, "Attention", CL.custom_min:format(L.submerge, 1))
	self:DelayedMessage("submerge", 150, "Attention", CL.custom_sec:format(L.submerge, 30))
	self:DelayedMessage("submerge", 170, "Urgent", CL.custom_sec:format(L.submerge, 10), false, "Alarm")
	self:DelayedMessage("submerge", 175, "Urgent", CL.custom_sec:format(L.submerge, 5), false, "Alarm")
end
...
function mod:Emerge()
	...
	self:Message("emerge", "Attention", "Long", L.emerge_message, "misc_arrowlup")
	self:Bar("submerge", 180, L.submerge_bar, "misc_arrowdown")
	self:DelayedMessage("submerge", 60, "Attention", CL.custom_min:format(L.submerge, 2))
	self:DelayedMessage("submerge", 120, "Attention", CL.custom_min:format(L.submerge, 1))
	self:DelayedMessage("submerge", 150, "Attention", CL.custom_sec:format(L.submerge, 30))
...
function mod:SonDeaths()
	sonsdead = sonsdead + 1
	if sonsdead < 9 then
		self:Message("emerge", "Positive", nil, CL.add_killed:format(sonsdead, 8), "spell_fire_elemental_totem")
	end
	if sonsdead == 8 then
